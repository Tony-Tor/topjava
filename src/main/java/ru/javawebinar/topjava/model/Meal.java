...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.BETWEEN_HALF_OPEN, query = "SELECT m FROM Meal m WHERE m.dateTime>:start_data_time AND m.dateTime<:end_data_time AND m.user.id=:user_id ORDER BY m.dateTime DESC "),
})
@Entity
